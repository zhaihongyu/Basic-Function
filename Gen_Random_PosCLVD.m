...
function [New_Random_PosCLVDs_6xN]=Gen_Random_PosCLVD()
...
PosCLVD_Ratio=ones(Random_RowNum,1)-rand(Random_RowNum,1)*0.3;
% 
DC_Ratio=zeros(Random_RowNum,Random_ColNum);
for i=1:Random_RowNum
    DC_Ratio(i,:)=rand(1,Random_ColNum)*(1-PosCLVD_Ratio(i));
end
% 
ISO_Ratio=zeros(Random_RowNum,Random_ColNum);
for i=1:Random_RowNum
    ISO_Ratio(i,1:Random_ColNum/2)=ones(1,Random_ColNum/2)*(1-PosCLVD_Ratio(i))-DC_Ratio(i,1:Random_ColNum/2);
    ISO_Ratio(i,Random_ColNum/2+1:Random_ColNum)=...
        DC_Ratio(i,Random_ColNum/2+1:Random_ColNum)-ones(1,Random_ColNum/2)*(1-PosCLVD_Ratio(i));
end
...
for i=1:Random_RowNum
    for j=1:Random_ColNum
        Random_ISOs{i,j}=PosCLVD_Ratio(i)*M_CLVD_Pos+DC_Ratio(i,j)*M_DC+ISO_Ratio(i,j)*M_ISO_Pos;
    end
...
New_Random_PosCLVDs_6xN=Random_ISOs_6xN(:,Idx);
